...
def find_overlap(column1, column2, column_size):
    ...
    return (c1brs & ~c2brs & ~c1urs & ~c2urs) | \
           (~c1brs & c2brs & ~c1urs & ~c2urs) | \
           (~c1brs & ~c2brs & c1urs & ~c2urs) | \
           (~c1brs & ~c2brs & ~c1urs & c2urs)
...
def answer(g):
    ...
    previous_column_states = {previous_state: 1 for previous_state in 
                              range(1 << (column_count+1))}
    ...
    for column_value in columns_as_binary_numbers:
        ...
        for previous_column_state in previous_column_states:
            for overlapping_column_state in \
            overlapping_column_states_cache[(column_value, previous_column_state)]:
                next_column[overlapping_column_state] += \
                    previous_column_states[previous_column_state]
